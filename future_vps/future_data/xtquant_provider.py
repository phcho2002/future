...
token 读取顺序（VPS 友好，不写死路径）：
    1. 环境变量 XT_TOKEN
    2. future_data.paths.token_path() 指向的 xt_token.py
...
def _load_token() -> str:
    """读取迅投 token：环境变量 XT_TOKEN 优先，否则读 xt_token.py。

    路径解析（VPS 友好）：
        1. 环境变量 ``XT_TOKEN``：直接返回其值。
        2. 环境变量 ``XT_TOKEN_PATH``：指向自定义的 xt_token.py。
        3. ``future_data.paths.token_path()``：默认 ``<future_vps>/xt_token.py``。
        4. 兜底：本文件上两级目录（future_vps 根）下的 xt_token.py。

    xt_token.py 内容形如：  XT_TOKEN = "your_token_string"
    """
    ...
    env_path = os.environ.get("XT_TOKEN_PATH")
    if env_path:
        candidates.append(Path(env_path).expanduser())
    try:
        from future_data.paths import token_path

        candidates.append(token_path())
    except Exception:
        pass
    # 兜底：future_vps 根（本文件上两级）的 xt_token.py
    candidates.append(Path(__file__).resolve().parent.parent / "xt_token.py")
    ...
    raise FileNotFoundError(
        "未找到迅投 token。请设置环境变量 XT_TOKEN，或 XT_TOKEN_PATH 指向含 "
        'XT_TOKEN = "your_token" 的 xt_token.py，'
        "或在 <future_vps>/xt_token.py 中写入 token。"
    )
