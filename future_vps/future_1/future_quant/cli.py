...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run future-quant analysis with TqSdk data.")
    parser.add_argument("--symbol", default="IF0", help="DB futures symbol, e.g. IF0, RB0, M0")
    parser.add_argument(
        "--exchange",
        default="cffex",
        help="Exchange code (cffex/shfe/dce/czce/gfex/ine); required to qualify the TqSdk symbol",
    )
