...
parquet 缓存）。现已把这些逻辑统一到全系统共享的 ``future_data``
包（见 future_data/README.md）。本文件退化为一个**兼容垫片**，
...
但它只把 future_quant 自身加入路径，不会带上 future_vps 根目录，所以这里显式补上，
...
# 让 sibling 包 future_data 可被 import（脚本所在目录的上三级即 future_vps 根目录）
_VPS_ROOT = Path(__file__).resolve().parents[3]  # .../future_1/future_quant/data -> future_vps
