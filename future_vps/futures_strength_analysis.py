"""
期货强弱分析 - 统一引擎（VPS 版，读 futures_top40.json）
=========================================================================
...
合约清单：从 futures_top40.json 读取（已含 exchange），无需数据库。
...
# ---- 路径中心：脚本就在 future_vps 根目录，把自身目录加入 sys.path 即可 import future_data ----
...
try:
    ...
    from future_data import DEFAULT_JSON_PATH as _DEFAULT_SYMBOLS_JSON
    _HAS_FUTURE_DATA = True
except Exception:  # noqa: BLE001
    _HAS_FUTURE_DATA = False
    _DEFAULT_SYMBOLS_JSON = _SCRIPT_DIR / "futures_top40.json"

import akshare as ak

# 结果数据库：脚本同目录下的独立 strength.db（save_results 自动建表）
DB_PATH = str(_SCRIPT_DIR / "strength.db")
...
def _period_prefix(period):
    """period -> 结果表名前缀。1440 用 daily，其余按分钟约定（60 用 hourly）。"""
    p = str(period)
    if p == "1440":
        return "daily"
    if p == "60":
...
def fetch_klines(symbol, exchange, period, length):
    ...
    主路径：future_data.get_klines（默认 xtquant + TTL 缓存）。
...
# ============================================================
# 3. 合约清单层（从 futures_top40.json 读取）
# ============================================================

def get_universe(top=40, db_path=DB_PATH, json_path=None):
    """读取品种清单：从 futures_top40.json 读取（含 exchange）。

    JSON 结构为 list[dict]，含 排名/symbol/name/exchange/最新价格/最新持仓量 等字段。
    路径默认走 future_data.DEFAULT_JSON_PATH（环境变量 FUTURES_TOP40_JSON 可覆盖）。
    """
    json_path = json_path or str(_DEFAULT_SYMBOLS_JSON)
    import json
    ...
    # 按 排名 升序，保留原始字段（最新价格/最新持仓量 等供下游使用）
    if "排名" in df.columns:
        df = df.sort_values("排名").reset_index(drop=True)
    # 保证 exchange 列存在（个别下游脚本 .get("exchange") 用）
...
# ============================================================
# 6. 存储层（独立 strength.db，表名按周期隔离，自动建表）
# ============================================================
...
# 快照追踪表列定义（run_time 作为主键，使 INSERT OR REPLACE 语义有效）。
# 列名与原 Windows 版兼容；VALUES 占位符由本列表长度驱动，避免手数出错。
_TRACK_COLUMNS = [
    "run_time", "run_label",
...
def _ensure_tracking_table(cur, table):
    """建 top3_tracking 快照表（首次写入前），schema 与原版兼容。"""
    cols_def = ", ".join(
        [f"{_TRACK_COLUMNS[0]} TEXT PRIMARY KEY"]
        + [f"{c} TEXT" for c in _TRACK_COLUMNS[1:2]]
        + [f"{c} TEXT" for c in _TRACK_COLUMNS[2:]]
    )
...
def save_results(results_df, prefix, db_path=DB_PATH, run_key=None, period_label=""):
    ...
    try:
        ...
        # 快照追踪表：首次建表，之后 INSERT OR REPLACE
        ...
        if len(s) >= 3 and len(w) >= 3:
            try:
                _ensure_tracking_table(cur, tn["track"])
                cols = ", ".join(_TRACK_COLUMNS)
                placeholders = ", ".join(["?"] * len(_TRACK_COLUMNS))
                run_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                vals = [
                    run_time, period_label,
                ...
                assert len(vals) == len(_TRACK_COLUMNS), \
                    f"tracking vals {len(vals)} != cols {len(_TRACK_COLUMNS)}"
                cur.execute(
                    f"INSERT OR REPLACE INTO {tn['track']} ({cols}) VALUES ({placeholders})",
                    vals,
                )
...
def render_report(results_df, strong3, weak3, period_label, errors, db_path, report_path=None):
    """终端打印摘要 + 写 UTF-8 报告文件到 output/ 目录。"""
    lines = []
    lines.append("=" * 90)
    ...
    if report_path is None:
        ...
        report_path = str(out_dir / f"strength_report_{safe}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
...
def run(period="1440", top=40, length=None, db_path=DB_PATH, save=True, render=True):
    ...
    print(f"  数据源: {'akshare only' if _AKSHARE_ONLY else ('future_data(xtquant+缓存)' if _HAS_FUTURE_DATA else 'akshare')} | "
          f"length={length} | top={top}")

    universe = get_universe(top=top, db_path=db_path)
    print(f"\n从 futures_top40.json 读取 {len(universe)} 个品种")
...
def main():
    ...
    parser.add_argument("--db", default=DB_PATH, help="结果数据库路径")
