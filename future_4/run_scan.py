...
run_scan.py
===========
主入口: 用当前 config 中的默认参数扫描全部品种的最新有效信号及预警信号，
输出:
  - output/signals_latest.csv : 最新信号排行(按新鲜度/星级)
  - output/alerts_latest.csv  : 最新预警信号(缠绕带内贴近均线、可能突破)
...
def _enrich_latest_alert(df: pd.DataFrame, p: dict) -> pd.DataFrame:
    """对每个品种，取最新一条预警信号并附加行情上下文。"""
    d = attach_indicators(df, p)
    alerts = find_alerts(df, p)
    if not len(alerts):
        return pd.DataFrame()
    last = alerts.iloc[-1].copy()
    ai = int(last["cross_idx"])
    last["last_close"] = float(d["close"].iloc[-1])
    last["last_time"] = str(d["datetime"].iloc[-1])
    last["bars_since"] = len(d) - 1 - ai
    last["ema_fast"] = float(d["ema_fast"].iloc[-1])
    last["ema_slow"] = float(d["ema_slow"].iloc[-1])
    return pd.DataFrame([last])


def scan_alerts(cfg: Optional[dict] = None, table: Optional[str] = None) -> pd.DataFrame:
    """扫描全市场最新预警信号（使用注入模式滚动窗口，只关注当前状态）。"""
    cfg = cfg or load_config()
    p = cfg["strategy"]
    syms = read_symbols(cfg, table or cfg["data"]["symbol_table"])
    name_map = dict(zip(syms["symbol"], syms["name"]))
    ...
    for i, row in enumerate(syms.itertuples(index=False), 1):
        ...
        if df_scan is None or df_scan.empty:
            continue
        rec = _enrich_latest_alert(df_scan, p)
        if len(rec):
            rec.insert(0, "symbol", sym)
            rec.insert(1, "name", name_map.get(sym, ""))
            rows.append(rec)
        print(f"[{i}/{n}] {sym} {name_map.get(sym,'')}: alert={len(rec)}")
    if not rows:
        return pd.DataFrame()
    out = pd.concat(rows, ignore_index=True)
    # 越新鲜、越贴近均线（dist_fast+dist_slow 越小）排越前
    out["dist_sum"] = out["dist_fast"] + out["dist_slow"]
    out = out.sort_values(["bars_since", "dist_sum"], ascending=[True, True]).reset_index(drop=True)
    out = out.drop(columns=["dist_sum"])
    return out
...
def main():
    ...
    # ---- 预警信号 ----
    print("\n=== 扫描预警信号 ===")
    alert_df = scan_alerts(cfg, args.table)
    if len(alert_df):
        alert_path = os.path.join(out_dir, "alerts_latest.csv")
        alert_df.to_csv(alert_path, index=False)
        print(f"\n最新预警已保存: {alert_path} ({len(alert_df)} 条)")
        ac = ["symbol", "name", "datetime", "direction", "bars_since",
              "entry_price", "last_close", "dist_fast", "dist_slow", "twist_cross"]
        ac = [c for c in ac if c in alert_df.columns]
        print(alert_df[ac].to_string(index=False))
    else:
        print("无有效预警。")
