...
def _local_extrema(series: pd.Series, lookback: int) -> pd.Series:
    ...
    lo = arr[: n - 2 * lookback]
    # 以 i 为中心的窗口 [i-lookback .. i+lookback]
    # 用滑动窗口最小/最大比较中心元素
    # 构造中心序列 c = arr[lookback : n-lookback]
    c = arr[lookback : n - lookback]
    # 对每个窗口求 min/max: 用滑动窗口
    from numpy.lib.stride_tricks import sliding_window_view

    win = sliding_window_view(arr, 2 * lookback + 1)  # shape (n-2*lookback, 2*lookback+1)
...
def _rolling_extrema_monotone(ext_flags: np.ndarray, vals: np.ndarray, window: int, want_up: bool) -> np.ndarray:
    ...
    向量化: 用滑动窗口切片 + 在每个窗口内取极值点比较。
    """
    n = len(vals)
    out = np.zeros(n, dtype=bool)
    if n < 2:
        return out
    # 把极值点的值序列预先算好(只对极小/极大位置)
    from numpy.lib.stride_tricks import sliding_window_view

    # 窗口对 ext_flags 和 vals 的滑窗
    w = min(window, n)
    # 用滑窗覆盖所有结束位置 i (从 i=w-1 起); 对前 w-1 个用膨胀窗口
