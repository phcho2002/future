...
optimize.py
===========
对 Top40 品种做网格搜索，按利润因子选出全局最优参数组合，写回 config.yaml。
...
评估:
  ...
  - 副指标: 期望R、最大回撤R
产出:
  - output/optimization_report.csv : Top10 全局组合 + 每品种最优
  - 把全局最优写回 config.yaml 的 strategy 段 (ema_fast/ema_slow/vol_factor/body_pct)

性能:
  - K线只读一次(从 parquet 缓存)，参数循环纯内存计算
  - 不做多进程(pandas/numpy 已向量化, 单进程足够, 避免akshare线程问题)
"""
...
def _precompute_bonus(klines_all: dict, strat_base: dict) -> dict:
    ...
    返回 {symbol: {'higher_low':..., 'lower_high':...}}。"""
...
def _build_params(strat_base: dict, combo: dict, fixed: dict) -> dict:
    """合成一组完整 strategy 参数"""
    p = dict(strat_base)  # 保留 twist_band_ratio / bonus / swing_lookback 等
    p.update(combo)
    p.update(fixed)
    # 确保 bonus / swing 仍在
...
def write_back_config(config_path: str, best: dict) -> None:
    """把最优参数写回 config.yaml 的 strategy 段。
    用 yaml 解析+回写会丢注释, 故采用按行精确替换: 保留行尾注释, 并保证值与注释间有空格。"""
    import re
    ...
    repl = {k: cast(k, best[k]) for k in ["ema_fast", "ema_slow", "vol_factor", "body_pct"]}
    with open(config_path, "r", encoding="utf-8") as f:
        lines = f.readlines()
    out = []
    for line in lines:
        m = re.match(r"^(\s*)(ema_fast|ema_slow|vol_factor|body_pct):(\s*)(\S+)(\s*)(#.*)?$", line)
        if m and m.group(2) in repl:
            ...
            if comment and not comment.startswith(" "):
                comment = " " + comment
            out.append(f"{indent}{key}:{sp1}{repl[key]}{comment}\n" if comment else f"{indent}{key}:{sp1}{repl[key]}\n")
        else:
            out.append(line)
...
def run_optimization(config_path: str = "config.yaml", write_back: bool = True) -> dict:
    ...
    print("\n===== 全局 Top10 =====")
    cols = ["ema_fast", "ema_slow", "vol_factor", "body_pct", "n_trades", "win_rate", "profit_factor", "expectancy_r", "max_dd_r"]
    print(res_df.head(10)[cols].to_string(index=False))
...
def finalize_from_checkpoint(config_path: str = "config.yaml", write_back: bool = True) -> dict:
    ...
    # best_trades: 重跑全局最优组合得到交易明细
    best_row = res_df.iloc[0]
    best = {k: best_row[k] for k in ["ema_fast", "ema_slow", "vol_factor", "body_pct"]}
...
def _per_symbol_optimal(klines_all, top_rows, strat_base, fixed, bt_p, metric, pre=None):
    ...
    for sym, df in klines_all.items():
        ...
        for _, r in top_rows.iterrows():
            combo = {k: r[k] for k in ["ema_fast", "ema_slow", "vol_factor", "body_pct"]}
            p = _build_params(strat_base, combo, fixed)
            try:
                trades, _ = backtest(df, p, bt_p, precomputed=pre.get(sym) if pre else None)
            except Exception:  # noqa: BLE001
