"""tqsdk 行情数据通路自检脚本
==========================
验证 future_data 包（tqsdk 后端）能否正常拉取 K 线，分三层：

  1. 鉴权 + 单品种拉取（fetch_kline）
  2. 批量拉取（fetch_many，单连接多品种）
  3. 缓存层（get_klines 第二次走 TTL 缓存）

用法：python test_tqsdk.py
...
def test_auth():
    """1. 鉴权文件能否解析。"""
    print("\n[1/4] 鉴权文件 tqsdk.py 解析...")
    try:
        from future_data.provider import _load_auth
        auth = _load_auth()
        if auth is None:
            _fail("TqAuth 解析为 None")
            return False
        _ok(f"TqAuth 已加载（user={getattr(auth, '_user', '***')}）")
        return True
...
def test_batch():
    ...
    try:
        ...
        if missing:
            ...
            return False, out
        _ok(f"3/3 品种全部成功，耗时 {dt:.1f}s（单连接）")
