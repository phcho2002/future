#!/usr/bin/env python3
"""TOP40 fakebreak scan — 30min K线 (假突破反转).

使用 quote_cache parquet 数据，30分钟周期。
窗口参数按 60m→30m 等比放大 (60m参数×2)。
"""
...
def main():
    ...
    for s in symbols:
        sym = s["symbol"]
        f = cache_dir / f"{sym}_30m.parquet"
        if f.exists():
            try:
    ...
    if all_rows:
        out = _HERE / "fakebreak_30m_signals.csv"
