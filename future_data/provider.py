...
# ---------------------------------------------------------------- defaults
# 全系统共享缓存目录（不再写死 future_1/data_cache）。各系统可通过参数覆盖。
...
def _load_auth():
    """从 D:/work_ai/tq_auth.py 读取 TqAuth（进程内缓存）。
    ...
    auth_path = Path("D:/work_ai/tq_auth.py")
    if not auth_path.exists():
        raise FileNotFoundError(
            f"tq_auth.py 未找到: {auth_path}（需要 TqAuth 账号才能拉取 tqsdk 行情）"
        )

    text = auth_path.read_text(encoding="utf-8")
    m = re.search(r'TqAuth\("([^"]+)",\s*"([^"]+)"\)', text)
    if not m:
        raise ValueError(f"无法从 {auth_path} 解析 TqAuth(user, password)")
