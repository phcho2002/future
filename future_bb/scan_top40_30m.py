#!/usr/bin/env python3
"""future_bb 突破系统 — 全品种扫描 (akshare 30m)

读取 futures_top40.json 品种池，使用 akshare 获取30分钟K线数据，
运行突破检测 + 过滤条件，输出最新交易信号。
"""
import os
import sys
import json
...
for r in raw.get("symbols", []):
    if isinstance(r, (list, tuple)) and len(r) >= 3:
        symbols.append({"symbol": r[0], "name": r[1], "exchange": r[2]})

print(f"{'='*70}")
print(f"  future_bb 突破系统 — 全品种扫描")
print(f"  数据源: akshare (30m) | 品种: {len(symbols)} 个")
...
def get_30m_data(symbol, use_cache=True):
    """获取30分钟K线数据"""
    cache_path = _HERE / "cache" / f"{symbol}_minute_30.pkl"
...
for i, sym in enumerate(symbols):
    ...
    try:
        ...
        if long_sig or short_sig:
            ...
            mandatory = bool(latest.get("mandatory_pass_long", False))
...
if all_signals:
    ...
    print(f"\n  【做多信号】({len(longs)} 个)")
    print(f"  {'品种':6s} {'名称':12s} {'价格':>10s} {'ATR':>8s} {'ADX':>5s} {'分':>4s} {'RR':>4s}")
    for s in longs:
        print(f"  {s['code']:6s} {s['name'][:12]:12s} {s['price']:10.1f} {s['atr']:8.1f} {s['adx']:5.0f} {s['score']:4.0f} {s['rr']:4.2f}")
